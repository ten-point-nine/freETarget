...
int json_aux_port_enable; // Enable the AUX port only if there is Aux hardware
                          // installed
...
const json_message_t JSON[] = {
    ...
    {"\"ANGLE\":", &json_sensor_angle, IS_INT32, 0, NONVOL_SENSOR_ANGLE, 45,
     0}, // Locate the sensor angles
    {"\"AUX_PORT_ENABLE\":", &json_aux_port_enable, IS_INT32, 0,
     NONVOL_AUX_PORT_ENABLE, 0, 6},                 // Enable the AUX Port
    {"\"BYE\":", 0, IS_INT32, &bye, 0, 0, 0},       // Shut down the target
    {"\"ECHO\":", 0, IS_VOID, &show_echo, 0, 0, 0}, // Echo test
    {"\"FACE_STRIKE\":", &json_face_strike, IS_INT32, 0, NONVOL_FACE_STRIKE, 0,
     0}, // Face Strike Count
    {"\"FOLLOW_THROUGH\":", &json_follow_through, IS_INT32, 0,
     NONVOL_FOLLOW_THROUGH, 0, 0}, // Three second follow through
    {"\"INIT\"", 0, 0, IS_VOID, &init_nonvol, 0,
     0}, // Initialize the NONVOL memory
    {"\"KEEP_ALIVE\":", &json_keep_alive, IS_INT32, 0, NONVOL_KEEP_ALIVE, 120,
     0}, // TCPIP Keep alive period (in seconds)
    {"\"LED_BRIGHT\":", &json_LED_PWM, IS_INT32, &set_LED_PWM_now,
     NONVOL_LED_PWM, 50, 0},                    // Set the LED brightness
    {"\"MFS?", 0, 0, IS_VOID, &mfs_show, 0, 0}, // Display the MFS settings
    {"\"MFS_TAP_1\":", &json_mfs_tap_1, IS_MFS, 0, NONVOL_MFS_TAP_A, PAPER_SHOT,
     2},
    {"\"MFS_TAP_2\":", &json_mfs_tap_2, IS_MFS, 0, NONVOL_MFS_TAP_B, TARGET_ON,
     2},
    {"\"MFS_HOLD_1\":", &json_mfs_hold_1, IS_MFS, 0, NONVOL_MFS_HOLD_A,
     PAPER_FEED, 2},
    {"\"MFS_HOLD_2\":", &json_mfs_hold_2, IS_MFS, 0, NONVOL_MFS_HOLD_B,
     TARGET_OFF, 2},
    {"\"MFS_HOLD_12\":", &json_mfs_hold_12, IS_MFS, 0, NONVOL_MFS_HOLD_AB,
     LED_ADJUST, 2},
    {"\"MFS_HOLD_C\":", &json_mfs_hold_c, IS_MFS, 0, NONVOL_MFS_HOLD_C,
     NO_ACTION, 2},
    {"\"MFS_HOLD_D\":", &json_mfs_hold_d, IS_MFS, 0, NONVOL_MFS_HOLD_D,
     NO_ACTION, 2},
    {"\"MFS_SELECT_CD\":", &json_mfs_select_cd, IS_MFS, 0, NONVOL_MFS_SELECT_CD,
     NO_ACTION, 2},

    {"\"MIN_RING_TIME\":", &json_min_ring_time, IS_INT32, 0,
     NONVOL_MIN_RING_TIME, 500, 0}, // Minimum time for ringing to stop (ms)
    {"\"NAME_ID\":", &json_name_id, IS_INT32, &show_names, NONVOL_NAME_ID, 0,
     0}, // Give the board a name
    {"\"NAME_TEXT\":", (int *)&json_name_text, IS_TEXT + SSID_SIZE, &show_names,
     NONVOL_NAME_TEXT, 0, 8}, // Override the fixed nameIDs
    {"\"PAPER_ECO\":", &json_paper_eco, IS_INT32, 0, NONVOL_PAPER_ECO, 0,
     0}, // Ony advance the paper is in the black
    {"\"PAPER_SHOT\":", &json_paper_shot, IS_INT32, 0, NONVOL_PAPER_SHOT, 0,
     5}, // How many shots before advancing paper
    {"\"PAPER_TIME\":", &json_paper_time, IS_INT32, 0, NONVOL_PAPER_TIME, 500,
     0}, // Set the paper advance time
    {"\"PCNT_LATENCY\":", &json_pcnt_latency, IS_INT32, 0, NONVOL_PCNT_LATENCY,
     0, 1}, // Interrupt latency for PCNT adjustment
    {"\"POWER_SAVE\":", &json_power_save, IS_INT32, 0, NONVOL_POWER_SAVE, 0,
     0}, // Set the power saver time
    {"\"RAPID_COUNT\":", &json_rapid_count, IS_INT32, 0, 0, 0,
     0}, // Number of shots expected in series
    {"\"RAPID_ENABLE\":", &json_rapid_enable, IS_INT32, 0, 0, 0,
     0}, // Enable the rapid fire fieature
    {"\"RAPID_TIME\":", &json_rapid_time, IS_INT32, 0, 0, 0,
     0}, // Set the duration of the rapid fire event and start
    {"\"RAPID_WAIT\":", &json_rapid_wait, IS_INT32, 0, 0, 0,
     0}, // Delay applied between enable and ready
    {"\"SEND_MISS\":", &json_send_miss, IS_INT32, 0, NONVOL_SEND_MISS, 0,
     0}, // Enable / Disable sending miss messages
    {"\"SENSOR\":", (int *)&json_sensor_dia, IS_FLOAT, 0, NONVOL_SENSOR_DIA,
     232000, 0}, // Generate the sensor postion array
    {"\"SN\":", &json_serial_number, IS_FIXED, 0, NONVOL_SERIAL_NO, 0xffff,
     0}, // Board serial number
    {"\"START\"", 0, IS_VOID, &start_new_session, 0, 0,
     0}, // Start a new session
    {"\"STEP_COUNT\":", &json_step_count, IS_INT32, 0, NONVOL_STEP_COUNT, 0,
     0}, // Number of pulses to send when running
    {"\"STEP_RAMP\":", &json_step_ramp, IS_INT32, 0, NONVOL_STEP_RAMP, 0,
     4}, // Interval (in ms) to ramp the stepper motor
    {"\"STEP_START\":", &json_step_start, IS_INT32, 0, NONVOL_STEP_START, 0,
     4}, // Starting interval (in ms) for the stepper moro
    {"\"STEP_TIME\":", &json_step_time, IS_INT32, 0, NONVOL_STEP_TIME, 0,
     0}, // Interval (in ms) between pulses when running
    {"\"TABATA_ENABLE\":", &json_tabata_enable, IS_INT32, 0, 0, 0,
     0}, // Enable the tabata feature
    {"\"TABATA_ON\":", &json_tabata_on, IS_INT32, 0, 0, 0,
     0}, // Time that the LEDs are ON for a Tabata timer (1/10 seconds)
    {"\"TABATA_REST\":", &json_tabata_rest, IS_INT32, 0, 0, 0,
     0}, // Time that the LEDs are OFF for a Tabata timer
    {"\"TABATA_WARN_OFF\":", &json_tabata_warn_off, IS_INT32, 0, 0, 0,
     0}, // Time that the LEDs are ON during a warning cycle
    {"\"TABATA_WARN_ON\":", &json_tabata_warn_on, IS_INT32, 0, 0, 200,
     0}, // Time that the LEDs are OFF during a warning cycle
    {"\"TARGET_TYPE\":", &json_target_type, IS_INT32, 0, NONVOL_TARGET_TYPE, 0,
     0}, // Marify shot location (0 == Single Bull)
    {"\"TEST\":", 0, IS_INT32, &self_test, 0, 0, 0}, // Execute a self test
    {"\"TOKEN\":", &json_token, IS_INT32, 0, NONVOL_TOKEN, 0,
     0}, // Token ring state
    {"\"TRACE\":", 0, IS_INT32, &set_trace, 0, 0,
     0}, // Enter / exit diagnostic trace
    {"\"VERSION\":", 0, IS_INT32, &POST_version, 0, 0,
     0}, // Return the version string
    {"\"VREF_LO\":", (int *)&json_vref_lo, IS_FLOAT, &set_VREF, NONVOL_VREF_LO,
     1250, 0}, // Low trip point value (Volts)
    {"\"VREF_HI\":", (int *)&json_vref_hi, IS_FLOAT, &set_VREF, NONVOL_VREF_HI,
     2000, 0}, // High trip point value (Volts)
    {"\"WIFI_CHANNEL\":", &json_wifi_channel, IS_INT32, 0, NONVOL_WIFI_CHANNEL,
     6, 0}, // Set the wifi channel
    {"\"WIFI_HIDDEN\":", &json_wifi_hidden, IS_INT32, 0, NONVOL_WIFI_HIDDEN, 0,
     1}, // Hide the SSID
    // {"\"WIFI_IP\":",        (int*)&json_wifi_ip,               0,
    // IS_TEXT+IP_SIZE, 0,          NONVOL_WIFI_IP,          0
    // },    // Static IP address
    {"\"WIFI_PWD\":", (int *)&json_wifi_pwd, IS_SECRET + PWD_SIZE, 0,
     NONVOL_WIFI_PWD, 0, 0}, // Password of SSID to attach to
    {"\"WIFI_RESET\":", &json_wifi_reset_first, IS_INT32, 0,
     NONVOL_WIFI_RESET_FIRST, 1,
     3}, // Reset everything on the first WiFI connection
    {"\"WIFI_SSID\":", (int *)&json_wifi_ssid, IS_TEXT + SSID_SIZE, 0,
     NONVOL_WIFI_SSID, 0, 0}, // Name of SSID to attach to
    {"\"X_OFFSET\":", (int *)&json_x_offset, IS_FLOAT, 0, NONVOL_X_OFFSET, 0,
     7}, // Correction to add to X to align target
    {"\"Y_OFFSET\":", (int *)&json_y_offset, IS_FLOAT, 0, NONVOL_Y_OFFSET, 0,
     7}, // Correction to add to Y to align target
    {"\"Z_OFFSET\":", &json_z_offset, IS_INT32, 0, NONVOL_Z_OFFSET, 13,
     0}, // Distance from paper to sensor plane (mm)
    ...
    {"\"TARGET_NAME\":", 0, IS_VOID, 0, 0, 0,
     0},                   // Target name for online version
    {0, 0, 0, 0, 0, 0, 0}, //
};
// clang-format on
      ...
      if (JSON[j].token != 0) {
        ...
        if (k > 0)          // Non zero, found something
        {
          ...
          case IS_INT32: // Convert an integer
            if ((input_JSON[i + k] == '0') &&
                ((input_JSON[i + k + 1] == 'X') ||
                 (input_JSON[i + k + 1] == 'x'))) // Is it Hex?
            {
              x = (to_int(input_JSON[i + k + 2]) << 4) +
                  to_int(input_JSON[i + k + 3]);
            } else {
...
void show_echo(void) {
  ...
  while (JSON[i].token != 0) // Still more to go?
  {
    if (JSON[i].value != NULL) // It has a value ?
    {
      ...
      case IS_MFS: // Covert to a switch ID
        ...
        if (mfs_ptr != NULL) {
          SEND(sprintf(_xs, "%-18s \"(%d) - %s\", ", JSON[i].token,
                       *JSON[i].value, mfs_ptr->text);)
        }
        break;
      ...
      case IS_FLOAT:
        SEND(sprintf(_xs, "%-18s %6.2f, ", JSON[i].token,
                     *(double *)JSON[i].value);)
        break;
      }
      vTaskDelay(10);
  ...
  SEND(sprintf(_xs, "\"TIME_TO_SLEEP\":     %4.2f,",
               (float)power_save /
                   (float)(ONE_SECOND * 60));) // How long until we sleep
  ...
  SEND(sprintf(_xs, "\"SPEED_OF_SOUND\":    %4.2f,",
               speed_of_sound(temperature_C(), humidity_RH()));)
  SEND(sprintf(_xs, "\"TIMER_COUNT\":       %d,",
               (int)(SHOT_TIME *
                     OSCILLATOR_MHZ));) // Maximum number of clock cycles to
  ...
  SEND(sprintf(_xs, "\"WiFi_MAC\":          \"%02X:%02X:%02X:%02X:%02X:%02X\",",
               str_c[0], str_c[1], str_c[2], str_c[3], str_c[4], str_c[5]);)
  ...
  if (json_wifi_ssid[0] == 0) // The SSID is undefined
  {
    ...
    SEND(sprintf(_xs,
                 "\"WiFi_MODE\":         \"Station connected to SSID %s\",",
                 (char *)&json_wifi_ssid);)
  }
