 ...
 * y Ratio of Specific Heats of Gas (Cp/Cv), varies with Temperature for Air @ Standard Atmospheic Pressure. y = 1.4 for an ideal diatomic gas.
 * R Universal Gas Constant. Revised 2019. Avogaro's number and the Boltzmann constant were given exact numerical values as was the gas constant. SI Units.
 * T Temperature in degrees Kelvin. (Use TK below.)
 * M Mean Molar Mass of Gas @ sea level, varies for Air due mainly to Humidity and CO2 content.
 *
...
)
{
  ...
  //int CO2_percent             // Calculated % CO2 (PPM) based on NOAA Data. Estimated valid to 2036. Accuracy < 1% error 1945~2036. Used to calculate Mean Molar Mass of Dry Air.
  ...
  // Calculate Coefficients to determine % change in speed of sound due to Humidity only, relative to Speed of Sound in Dry Air at 0 degrees C.
  ...
  speed_MPS = sqrt((y * R * TK) / M)                          // Speed, in Metres per second, of Sound in Dry Air at (temperature) degrees C.
              ...
              + ((speed_MPSTo / 100.0d) * change_in_speed);   //% Change in speed due to humidity, relative to Speed of Sound in Dry Air at 0 degrees C.


  speed_mmPuS = speed_MPS * TO_MM / TO_US;                    // Convert down to mm/us
  
  DZZ(DLT_DIAG, printf("Temperature: %4.2fC Humidity: %4.2f%% Speed of Sound: %4.2fmm/us", temperature, relative_humidity, speed_MPS);)
