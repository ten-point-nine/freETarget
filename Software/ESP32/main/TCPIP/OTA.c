...
void OTA_load(void)
{
  ...
  {
    DLT(DLT_CRITICAL,
        SEND(ALL, sprintf(_xs, "Configured OTA boot partition at offset 0x%08" PRIx32 ", but running from offset 0x%08" PRIx32,
                          configured->address, running->address);))
    DLT(DLT_CRITICAL,
        SEND(ALL, sprintf(_xs, "(This can happen if either the OTA boot data or preferred boot image become corrupted somehow.)");))
  }

  DLT(DLT_CRITICAL, SEND(ALL, sprintf(_xs, "Running partition type %d subtype %d (offset 0x%08" PRIx32 ")", running->type, running->subtype,
                                      running->address);))
  ...
  DLT(DLT_CRITICAL, SEND(ALL, sprintf(_xs, "Writing to partition subtype %d at offset 0x%" PRIx32, update_partition->subtype,
    ...
    if ( data_read > 0 )                                                // Got data to read
    {
      if ( image_header_was_checked == false )
      {
        ...
        if ( data_read > sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t) + sizeof(esp_app_desc_t) )
        {
          ...
          {
            DLT(DLT_INFO, SEND(ALL, sprintf(_xs, "Run()%s", running_app_info.version);))
          }
  ...
  DLT(DLT_INFO, SEND(ALL, sprintf(_xs, "Restarting)");))
  esp_restart();
  return;
}
