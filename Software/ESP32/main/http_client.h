...
#if BUILD_HTTP
// esp_err_t _http_event_handler(esp_http_client_event_t *evt);    // HTTP Event
// Handler
void http_rest_with_url(char *url, int method,
                        char *payload); // Send the payload to the URL
#endif

#if BUILD_HTTPS
void https_with_url(char *url, int method,
                    char *payload); // HTTPS transfer to URL
...
void http_native_request(char *url, int method, char *payload,
                         int payload_length); // Low level API
