...
void NTP_master(void)
{
  ...
  SEND(TCPIP, sprintf(_xs, "{\"%s\":%lld}", _NTP_MASTER_, NTP_base_time);) // Send back the ack
  DLT(DLT_DEBUG, SEND(CONSOLE, sprintf(_xs, "{\"%s\":%lld}", _NTP_MASTER_, NTP_base_time);))
  return;
}
...
void NTP_slave(void)
{
  ...
  SEND(TCPIP, sprintf(_xs, "{\"%s\":%lld}", _NTP_SLAVE_, NTP_base_time);) // Send back the ack
  DLT(DLT_DEBUG, SEND(CONSOLE, sprintf(_xs, "{\"%s\":%lld}", _NTP_SLAVE_, NTP_base_time);))
  return;
}
...
void NTP_offset(void)
{
  ...
  DLT(DLT_DEBUG, SEND(CONSOLE, sprintf(_xs, "{\"NPT_OFFSET\":%lld}", NTP_offset_time);))
  return;
}
