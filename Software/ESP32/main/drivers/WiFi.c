...
void WiFi_init(void)
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "WiFi_init()");))
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "mDNS service set up for: \"%s\"", str_c);))
...
void WiFi_AP_init(void)
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "WiFi_AP_init()");))
...
void WiFi_station_init(void)
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "WiFi password: %s", json_wifi_pwd);))
...
)
{
  ...
  if ( dns_valid == 0 )
  {
    DLT(DLT_CRITICAL, SEND(CONSOLE, sprintf(_xs, "URL %s not found", remote_url);))
  }
  return dns_valid;
}
...
static void wifi_set_static_ip(esp_netif_t *netif)
{
  ...
  {
    DLT(DLT_CRITICAL, SEND(CONSOLE, sprintf(_xs, "Failed to stop dhcp client");))
    return;
  }
  ...
  {
    DLT(DLT_CRITICAL, SEND(CONSOLE, sprintf(_xs, "Failed to change IP address");))
    return;
  }
...
{
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "WiFi_tcp_server_task()");))
...
void tcpip_socket_poll_0(void *parameters)
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "tcp_socket_poll_0()");))
...
void tcpip_socket_poll_1(void *parameters)
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "tcp_socket_poll_1()");))
...
void tcpip_socket_poll_2(void *parameters)
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "tcp_socket_poll_2()");))
...
void tcpip_socket_poll_3(void *parameters)
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "tcp_socket_poll_3()");))
...
void tcpip_accept_poll(void *parameters)
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "tcp_accept_poll()");))
  ...
  if ( listen_sock < 0 )
  {
    DLT(DLT_CRITICAL, SEND(CONSOLE, sprintf(_xs, "Unable to create socket: errno %d\r\n", errno);))
    vTaskDelete(NULL);
    return;
  }
