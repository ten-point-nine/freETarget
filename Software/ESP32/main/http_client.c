...
*/
#if (BUILD_HTTPS)
const char howsmyssl_com_root_cert_pem_start[] asm(
    "_binary_howsmyssl_com_root_cert_pem_start");
const char howsmyssl_com_root_cert_pem_end[] asm(
 ...
 *------------------------------------------------------------*/
esp_err_t
_http_event_handler(esp_http_client_event_t *evt // Event being processed
) {
  ...
  case HTTP_EVENT_ON_HEADER:
    DLT(DLT_DIAG, printf("HTTP_EVENT_ON_HEADER, key=%s, value=%s",
...
void http_rest_with_url(char *url,    // URL being accessed
  ...
  case METHOD_PUT:
    ...
    if (err == ESP_OK) {
      ...
      DLT(DLT_CRITICAL,
          printf("HTTP PUT request failed: %s", esp_err_to_name(err));)
    }
    break;
#endif
  ...
  case METHOD_PATCH:
    ...
    if (err == ESP_OK) {
      ...
      DLT(DLT_CRITICAL,
          printf("HTTP PATCH request failed: %s", esp_err_to_name(err));)
    }
    break;
#endif
  ...
  case METHOD_DELETE:
    ...
    if (err == ESP_OK) {
      ...
      DLT(DLT_CRITICAL,
          printf("HTTP DELETE request failed: %s", esp_err_to_name(err));)
    }
    break;
#endif
  ...
  case METHOD_HEAD:
    ...
    if (err == ESP_OK) {
      ...
      DLT(DLT_CRITICAL,
          printf("HTTP HEAD request failed: %s", esp_err_to_name(err));)
    }
    break;
#endif
  }
...
void http_download_chunk(char *url, // URL to read
  ...
  if (err == ESP_OK) {
    DLT(DLT_DIAG,
        printf("HTTP chunk encoding Status = %d, content_length = %" PRId64,
               esp_http_client_get_status_code(client),
               esp_http_client_get_content_length(client));)
  } else {
    DLT(DLT_DIAG,
...
void http_perform_as_stream_reader(char *url, // URL to read from
  ...
  DLT(DLT_DIAG,
      printf("HTTP Stream reader Status = %d, content_length = %" PRId64,
             esp_http_client_get_status_code(client),
             esp_http_client_get_content_length(client));)
...
void https_async(char *url,    // URL to connect to
  ...
  if (err == ESP_OK) {
    DLT(DLT_DIAG, printf("HTTPS Status = %d, content_length = %" PRId64,
                         esp_http_client_get_status_code(client),
                         esp_http_client_get_content_length(client));)
  } else {
    DLT(DLT_DIAG,
...
void http_native_request(char *server_url,  // URL to be accessed
  ...
  esp_http_client_config_t config = {.url =
                                         "http://joshua.10nine.co/api/shots"};
  esp_http_client_handle_t client = esp_http_client_init(&config);
  ...
  case METHOD_GET:
    ...
    if (err != ESP_OK) {
      ...
      data_read = esp_http_client_read_response(client, output_buffer,
  ...
  case METHOD_POST:
    ...
    if (esp_http_client_open(client, strlen(payload)) != ESP_OK) {
      DLT(DLT_CRITICAL,
          printf("Failed to open HTTP connection: %s", server_url);)
    } else {
      ...
      if (esp_http_client_fetch_headers(client) < 0) {
        ...
        //                   i++){payload[i] = 0;})
        if (esp_http_client_read_response(client, payload, payload_length) >=
            0) {
...
void http_partial_download(char *url,         // URL to access
  ...
  if (esp_http_client_perform(client) == ESP_OK) {
    DLT(DLT_DIAG, printf("HTTP Status = %d, content_length = %" PRId64,
                         esp_http_client_get_status_code(client),
                         esp_http_client_get_content_length(client));)
  } else {
