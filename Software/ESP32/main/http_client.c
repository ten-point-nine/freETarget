...
(
    esp_http_client_event_t *evt    // Event being processed
)
{
...
)
{
    ...
    if (err == ESP_OK)
    {
        DLT(DLT_DIAG, printf("HTTP chunk encoding Status = %d, content_length = %"PRId64,
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));)
    } 
    else 
...
)
{
    ...
    DLT(DLT_DIAG, printf("HTTP Stream reader Status = %d, content_length = %"PRId64,
                    esp_http_client_get_status_code(client),
                    esp_http_client_get_content_length(client));)
