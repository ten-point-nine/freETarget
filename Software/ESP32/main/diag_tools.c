...
bool factory_test(void) {
   ...
   */
  SEND(sprintf(_xs, "\r\nFirmware version: %s   Board version: %d",
               SOFTWARE_VERSION, revision());)
  SEND(sprintf(_xs, "\r\n");)
  SEND(sprintf(
           _xs,
  ...
  while (1) {
    ...
    if (v12_supply() >=
        V12_WORKING) // Skip the motor and LED test if 12 volts not used
    {
      SEND(sprintf(_xs, "  M");)
      if (motor_toggle) {
        SEND(sprintf(_xs, "+");)
...
bool POST_counters(void) {
  ...
  if (is_running() != 0xFF) {
    DLT(DLT_CRITICAL,
        SEND(sprintf(_xs, "Failed to start clock in run latch: %02X",
                     is_running());))
...
bool do_dlt(unsigned int level) {
  ...
  if ((level & (is_trace | DLT_CRITICAL | DLT_INFO)) ==
      0) // DLT_CRITICAL are always set in is_trace
  {
