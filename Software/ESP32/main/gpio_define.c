 ...
 *  Variables
 */
const static char* gpio_use[] = {
    "DIGITAL_IO", "ANALOG_IO", "SERIAL_AUX", "PWM_OUT", "I2C_PORT", "PCNT", "LED_STRIP"}; 
 ...
 */
    ...
    while (gpio_table[i].gpio_name != 0 )
    {
        ...
        {   
            DLT(DLT_INFO,  
            SEND(sprintf(_xs, "%s: %d  %s",gpio_table[i].gpio_name, gpio_table[i].gpio_number, gpio_use[((const DIO_struct_t *)(gpio_table[i].gpio_uses))->type]);))
            ...
            {      
                ...
                case PWM_OUT:
                    pwm_init(((const PWM_struct_t*)(gpio_table[i].gpio_uses))->pwm_channel,\
